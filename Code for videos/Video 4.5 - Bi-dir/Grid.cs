...
namespace Video_4._5___Bi_dir
{
    class Grid
    {
        ...
        public List<Point> GetAdjacentCells2(int x, int y)
        {
        ...
        public List<Point> GetAdjacentCells(int x, int y, bool passableWalls = false)
        {
            List<Point> adjacentCells = new List<Point>();
            if (x > 0)
            {
                // Look left
                if (passableWalls || _grid[x - 1, y] == 1)
                {
                    adjacentCells.Add(new Point(x - 1, y));
                }
            }
            ...
            if (y > 0)
            {
                if (passableWalls || _grid[x, y - 1] == 1)
                {
                    adjacentCells.Add(new Point(x, y - 1));
                }
            }
